...
- `bounty_map` *(dict or None)*: Per-class reward multipliers. Default: `{1: 1.5, 2: 2.0, 7: 1.5, 8: 2.0}`.
...
Batch inference without updating balances or triggering evolution. Use on validation and test sets.
...
**2. Economic Incentive (Bounty)** — Rare classes carry higher `beta_c` multipliers, so agents that correctly detect them earn disproportionately more balance. This creates automatic economic pressure pulling ensemble capacity toward hard-to-detect minority classes — the agents that ignore rare classes fall behind in wealth and eventually get replaced, while those that specialise in detecting them accumulate capital and gain more influence over the final prediction.
